      ...
      if(all(c("Sample Name", "Sample Position")%in%colnames(temp))){
        temp = make.WL(filename = input$file1$datapath,
                       ...
                       sample.prefix = ifelse(input$development == F,
                                              paste(format(Sys.time(), "%Y%m%d"),sub("-.*", "", input$selectTQ),input$User,"runningID",input$FolderName,sep = "_"),
                                              paste(format(Sys.time(), "%Y%m%d"),sub("-.*", "", input$selectTQ),input$User,sep = "_")),
