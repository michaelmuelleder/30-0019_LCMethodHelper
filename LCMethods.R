...
methods.inUse = c("HILIC Amino acids (3.25 min)","HILIC Amino acids (12 min)","HILIC basic 100 mM (NH4)2CO3, on 100 mm","RP Proteomics 10 min","RP Nucleosides")
...
"RP Nucleosides" = 
  list(Gradient = data_frame(B2= c(rep(2,2),25,rep(80,2),rep(2,4)),
       ...
       WL.fun = list(filename = NULL,
                     ...
                     nSTD = 7,
                     brackets = 12)),
## RP Proteomics 10 min --------------------------------------------
"RP Proteomics 10 min" = 
  list(Gradient = data_frame(B2= c(rep(3,2),35,rep(98,2),rep(3,3)),
                             A2= 100 - B2,
                             Flow = rep(0.8,8),
                             Time = c(0,1,7.5,8,8.5,8.6,11,11.7)),
       SolventChannels = c("A2","B2"),
       SolventSystem = data.frame(Solvent = c("A1","B1","A2","B2","Needle Wash"),
                                  Composition = c("2:1 Water:Acetonitrile", 
                                                  "Acetonitrile",
                                                  ...
                                                  "2:1 Water:Acetonitrile")),
       Reagents = data.frame(Reagent = c("Formic Acid (>99%)","ACN","Water"),
                             `MW (g/mol)` = c(46.02538,NA,NA),
                             `Density (g/ml)` = c(1.22,NA,NA),
                             Concentration = c("100%", "100%", "100%")),
       Column = "InfinityLab Poroshell 120 EC-C18, 2.1 x 50 mm, 1.9 um",
       #Pressure = data.frame(Time = seq(0,3.95,0.05),
       #                     Pressure = 600),
       calcSolvents = function(fd){
         bind_rows(fd[fd$Solvent == "A2",] %>%
       ...
       WL.fun = list(filename = NULL,
                     sample.path = NULL,
                     method.path = file.path("D:","MassHunter","Methods","B.07.00","000_SOPs","RP-MS-Proteomics",fsep = "\\"),
                     analysis.method = c("RP-MS-Proteomics-10min-50mm.m"),
                     nSTD = 7,
                     brackets = 12))
)

Methods = Methods[methods.inUse]
